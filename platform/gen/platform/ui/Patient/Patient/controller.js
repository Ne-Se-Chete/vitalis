angular.module('page', ["ideUI", "ideView", "entityApi"])
	.config(["messageHubProvider", function (messageHubProvider) {
		messageHubProvider.eventIdPrefix = 'platform.Patient.Patient';
	}])
	.config(["entityApiProvider", function (entityApiProvider) {
		entityApiProvider.baseUrl = "/services/ts/platform/gen/platform/api/Patient/PatientService.ts";
	}])
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'platform-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Patient" && e.view === "Patient" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Patient", `Unable to list/filter Patient: '${response.message}'`);
						return;
					}
		...
		$http.get("/services/ts/platform/gen/platform/api/Settings/GenderService.ts").then(function (response) {
