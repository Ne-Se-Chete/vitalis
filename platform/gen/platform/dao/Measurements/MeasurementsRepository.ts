...
export class MeasurementsRepository {
    ...
    private async triggerEvent(data: MeasurementsEntityEvent | MeasurementsUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("platform-Measurements-Measurements", ["trigger"]);
        ...
        producer.topic("platform-Measurements-Measurements").send(JSON.stringify(data));
    }
}
